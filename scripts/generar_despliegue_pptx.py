...
# =====================================================================
deck.cover(
    "ANALISIS TECNICO  ·  VERSION 1.0",
    "DONDE DESPLEGAR",
    "KG Academy en produccion",
    "Comparacion de seis alternativas de alojamiento con costos, riesgos y esfuerzo de "
    "administracion, una recomendacion argumentada y el plan de puesta en produccion "
    "paso a paso.",
)

# =====================================================================
s = deck.page("Punto de partida", "Que necesita esta aplicacion para funcionar",
...
req = [
    ...
    ("Base de datos PostgreSQL", "Relacional, con 44 tablas e integridad referencial. MySQL tambien serviria, "
     "pero PostgreSQL es la eleccion recomendada.", NAVY_L),
    ("HTTPS con dominio propio", "Las cookies de sesion y los codigos QR de los certificados dependen del dominio "
     "definitivo. Sin SSL no hay plataforma.", LIME),
    ("Almacenamiento de video externo", "El video NO debe servirse desde el servidor de la aplicacion: consume todo "
     "el ancho de banda y degrada la plataforma.", AMBER),
    ("Respaldos automaticos", "La base contiene la evidencia de capacitacion. Perderla es perder los certificados "
     "y la trazabilidad ante una auditoria.", RED),
]
y = 1.8
...
text(s, Inches(9.55), Inches(2.0), Inches(2.95), Inches(4.2),
     [("DIMENSIONAMIENTO", 11, True, LIME, 10),
      ...
      ("Con ese tamano sobra holgadamente para el primer ano.", 8.5, False, GREY, 0)], spacing=1.15)
...
note(s, Inches(0.62), Inches(6.65), Inches(12.1),
     ...
     "recomendacion, pero conviene saber que plan de Hostinger sirve y cual no.", h=0.55)

# =====================================================================
s = deck.page("Advertencia", "Por que el hosting compartido NO sirve",
           "Es el error mas comun y el mas caro de descubrir tarde. Aplica a Hostinger y a cualquier proveedor.")
...
malo = [
    ...
    "El proceso se reinicia o se suspende por limites de CPU y memoria.",
    "No hay acceso de consola para PM2, migraciones ni tareas programadas.",
    "Si se sube la aplicacion como sitio estatico, dejan de funcionar el login, la base de datos y los certificados.",
]
y = 2.55
for m in malo:
...
bueno = [
    ...
    "Cualquiera de las dos rutas cumple; cambian el costo y quien administra.",
]
y = 2.55
for m in bueno:
...
note(s, Inches(0.62), Inches(5.6), Inches(12.1),
     ...
     "corporativo y el correo, pero la plataforma necesita ademas un VPS.", tono=LIME_L, h=0.6)
...
opciones = [
    ...
    ("Vercel + Neon", "US$ 20 - 40", "Muy bajo", "Medio", "ALTERNATIVA",
     "Cero administracion, ideal para Next.js, costo recurrente mayor"),
    ...
    ("DigitalOcean o Hetzner VPS", "US$ 6 - 15", "Medio", "Alto", "EQUIVALENTE",
     "Mismo esquema que el VPS de Hostinger, a veces mas potencia por el precio"),
    ("Servidor propio en KG", "Variable", "Muy alto", "Total", "NO RECOMENDADO",
     "Requiere IP fija, UPS, seguridad fisica y soporte 24/7"),
]
table(s, Inches(0.62), Inches(1.78), Inches(12.1),
      ["Alternativa", "Costo / mes", "Esfuerzo de administracion", "Control", "Veredicto", "Comentario"],
...
note(s, Inches(0.62), Inches(6.1), Inches(12.1),
     ...
     "El costo del dominio (.co, aproximadamente US$ 25 al ano) es aparte en todas las opciones.",
     tono=CLOUD, borde=LINE, h=0.6, size=9.5)

# =====================================================================
s = deck.page("Criterios", "Como se tomo la decision",
           "Cinco criterios, ponderados segun la realidad de KG: una PYME que lanza su primer producto digital.")
criterios = [
    ("Costo recurrente", "30 %", "Debe caber en el presupuesto de una PYME sin comprometer el proyecto.", LIME),
    ("Esfuerzo de administracion", "25 %", "KG no tiene un area de sistemas dedicada al mantenimiento del servidor.", NAVY_L),
    ("Cumplir el requisito del cliente", "20 %", "El documento funcional pide explicitamente Hostinger.", NAVY),
    ("Control y portabilidad", "15 %", "Poder migrar sin reescribir la aplicacion ni quedar atado al proveedor.", AMBER),
    ("Latencia desde Colombia", "10 %", "Elegir el centro de datos mas cercano disponible marca la diferencia.", RED),
]
y = 1.82
...
text(s, Inches(8.78), Inches(2.02), Inches(3.7), Inches(3.7),
     [("CONCLUSION", 11, True, LIME, 10),
      ("El VPS de Hostinger gana porque es el unico que satisface a la vez los tres criterios de mayor peso: "
       "es el mas economico de los viables, respeta el requisito del documento y no ata la plataforma a un proveedor.",
       10, False, WHITE, 12),
      ("Su unica debilidad es el esfuerzo de administracion, y se compensa dejando el servidor configurado, "
       "documentado y con respaldos automaticos desde el primer dia.", 9.5, False, GREY, 0)], spacing=1.2)

note(s, Inches(0.62), Inches(6.7), Inches(12.1),
     "Si KG prefiere no administrar ningun servidor, la segunda opcion es Vercel + Neon: cuesta aproximadamente el "
     "doble al mes pero elimina por completo el mantenimiento.", h=0.5)

# =====================================================================
s = deck.page("Recomendacion", "Arquitectura recomendada para el lanzamiento",
           "Hostinger VPS KVM 2, centro de datos mas cercano disponible (EE.UU. este o Sao Paulo).")

# Capas del despliegue
capas = [
    ("USUARIOS", "Trabajadores, empresas y KG  ·  navegador web  ·  HTTPS", LIME, NAVY),
    ("DOMINIO Y SSL", "kgacademy.co  ·  certificado Let's Encrypt con renovacion automatica", NAVY_L, WHITE),
    ("NGINX", "Proxy inverso en los puertos 80 y 443  ·  compresion  ·  cabeceras de seguridad", NAVY, WHITE),
    ("APLICACION", "Next.js en produccion  ·  proceso Node gestionado por PM2  ·  reinicio automatico", NAVY_D, WHITE),
    ("BASE DE DATOS", "PostgreSQL 16 en el mismo VPS  ·  solo accesible desde localhost", NAVY, WHITE),
    ("RESPALDOS", "pg_dump diario por cron  ·  retencion 30 dias  ·  copia fuera del servidor", NAVY_L, WHITE),
]
y = 1.78
...
note(s, Inches(0.62), Inches(6.7), Inches(12.1),
     ...
     "a internet, solo la alcanza la aplicacion que corre en el mismo servidor.", h=0.5, size=9.5)
...
costos = [
    ("VPS Hostinger KVM 2 (2 vCPU / 8 GB)", "US$ 8 - 18", "Incluido en Vercel + Neon", "Servidor de aplicacion y base de datos"),
    ("Plataforma gestionada (Vercel Pro)", "No aplica", "US$ 20", "Alojamiento de la aplicacion sin administracion"),
    ("PostgreSQL gestionado (Neon)", "No aplica", "US$ 0 - 19", "Base de datos con respaldos automaticos"),
    ("Dominio .co", "US$ 2 (25 al ano)", "US$ 2 (25 al ano)", "Direccion definitiva de la plataforma"),
    ("Certificado SSL", "US$ 0", "US$ 0", "Let's Encrypt o incluido en la plataforma"),
    ("Alojamiento de video", "US$ 0 - 12", "US$ 0 - 12", "Vimeo o Bunny Stream segun consumo"),
...
note(s, Inches(0.62), Inches(6.5), Inches(12.1),
     ...
     "mantenimiento (actualizaciones y revision de respaldos); la ruta B, practicamente ninguna.", h=0.6)

# =====================================================================
s = deck.page("Decision aparte", "Donde alojar el video de los cursos",
           "Es la decision tecnica que mas impacta el costo y la experiencia. Merece analisis propio.")
video = [
    ...
    ("Bunny Stream", "Recomendado", "Muy economico, se paga por consumo real, red de distribucion rapida en "
...
note(s, Inches(0.62), Inches(6.85), Inches(12.1),
     ...
     "Cambiar de proveedor de video mas adelante no exige tocar el codigo.", h=0.5, size=9.5)

# =====================================================================
s = deck.page("Cumplimiento", "Datos personales y respaldos",
           "Dos temas que suelen olvidarse hasta que hay una auditoria o una perdida de informacion.")
...
shape_text(rect(s, Inches(0.62), Inches(1.8), Inches(5.9), Inches(0.55), fill=NAVY),
           "PROTECCION DE DATOS  ·  LEY 1581 DE 2012", 10.5, True, WHITE)
legal = [
...
backups = [
    ("Diario", "pg_dump automatico a la 1:00 a.m., comprimido."),
...
note(s, Inches(0.62), Inches(6.62), Inches(12.1),
     ...
     "capacitacion que la empresa debe demostrar ante la ARL.", tono=CLOUD, borde=LINE, h=0.55, size=9.5)

# =====================================================================
s = deck.page("Ejecucion", "Plan de puesta en produccion",
           "Doce pasos. Un dia de trabajo tecnico si el dominio ya esta disponible.")
plan = [
    ...
    ("5", "Subir el codigo", "Clonar el repositorio en el servidor."),
    ...
    ("8", "Cargar los catalogos", "Roles, permisos, plantillas y configuracion. NO el seed de demostracion."),
    ("9", "Compilar y levantar", "npm run build y arrancar con PM2 con reinicio automatico."),
    ...
    ("12", "Pruebas de aceptacion", "Ingreso por cada rol, aula, evaluacion, certificado y QR con el dominio real."),
]
...
note(s, Inches(0.62), Inches(6.85), Inches(12.1),
     "Paso 8, critico: el seed actual BORRA la base antes de sembrar. En produccion debe ejecutarse solo la parte de "
     "catalogos, nunca el seed completo despues del lanzamiento.", tono=LIME_L, h=0.5, size=9.5)

# =====================================================================
s = deck.page("Ejecucion", "Detalles que suelen costar caro",
           "Errores frecuentes en el paso a produccion, y como evitarlos en este proyecto concreto.")
trampas = [
    ("El QR apunta a localhost", "Si NEXT_PUBLIC_APP_URL no tiene el dominio definitivo ANTES de emitir el primer "
     "certificado, los QR quedaran apuntando a localhost y habra que reemitirlos.", RED),
    ("La clave de sesion por defecto", "AUTH_SECRET debe cambiarse por una cadena larga y aleatoria. Con la clave de "
     "ejemplo, cualquiera podria falsificar una sesion.", RED),
    ("Ejecutar el seed en produccion", "Borraria usuarios, avances y certificados reales. Debe deshabilitarse el "
    ...
    ("No tener ambiente de pruebas", "Todo cambio deberia probarse en un subdominio de staging antes de tocar "
     "produccion. Un segundo VPS pequeno o un subdominio en el mismo servidor.", AMBER),
    ("Olvidar la zona horaria", "Configurar el servidor en America/Bogota para que las fechas de los certificados y "
...
# =====================================================================
s = deck.page("Futuro", "Que hacer cuando la plataforma crezca",
           "Senales concretas para saber cuando conviene cambiar de escalon, y a que escalon pasar.")
escalones = [
    ("HOY  ·  hasta 1.000 usuarios", LIME,
     ["Un VPS con la aplicacion y PostgreSQL juntos", "Respaldos diarios", "Monitoreo basico",
      "Costo aproximado: US$ 10 - 34 al mes"]),
    ("SENAL DE CAMBIO", AMBER,
     ["El servidor supera el 70 % de CPU o memoria de forma sostenida", "Las paginas tardan mas de 2 segundos",
      "Mas de 100 usuarios simultaneos", "La base supera los 20 GB"]),
    ("SIGUIENTE ESCALON", NAVY_L,
     ["Separar la base de datos en su propio servidor gestionado", "Subir el VPS a 4 vCPU / 16 GB",
      "Agregar una CDN para los archivos estaticos", "Costo aproximado: US$ 40 - 80 al mes"]),
    ("ESCALON MAYOR", NAVY,
     ["Dos servidores de aplicacion detras de un balanceador", "Replica de lectura de la base de datos",
...
note(s, Inches(0.62), Inches(6.65), Inches(12.1),
     "La arquitectura elegida permite subir de escalon sin reescribir nada: el mismo codigo funciona en un VPS "
     "pequeno y en una infraestructura con balanceador.", h=0.5)

# =====================================================================
s = deck.page("Resumen", "La recomendacion en una lamina",
...
text(s, Inches(1.0), Inches(2.0), Inches(11.4), Inches(1.2),
     [("RECOMENDACION", 11, True, NAVY, 4),
      ...
      ("Aproximadamente US$ 10 a 34 al mes, cumple el requisito del documento funcional y no ata la plataforma "
       "a ningun proveedor.", 10.5, False, NAVY, 0)], spacing=1.2)

razones = [
    ("Es viable", "Ejecuta Next.js con renderizado en servidor y PostgreSQL, que es lo que la aplicacion necesita."),
    ("Es economico", "La opcion mas barata entre las que realmente funcionan."),
    ("Respeta el requisito", "El esqueleto funcional pide despliegue en Hostinger."),
    ("Es portable", "Si manana KG quiere migrar, el mismo codigo corre en cualquier otro VPS o en Vercel."),
    ("Es suficiente", "2 vCPU y 8 GB cubren de sobra el primer ano de operacion."),
