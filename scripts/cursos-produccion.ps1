# ==========================================================================
# KG ACADEMY - Cursos en producción (Neon)
# --------------------------------------------------------------------------
# Crea los cursos del catálogo que todavía no existan en producción, EN
# BORRADOR, con sus módulos, lecciones y el material de Genially. No borra ni
# modifica nada y se puede repetir.
#
...
Write-Host ""
Write-Host "KG Academy - cursos en producción" -ForegroundColor Cyan
Write-Host ""
...
try {
  ...
  npx tsx prisma/cursos-produccion.ts
  $codigo = $LASTEXITCODE
}
finally {
...
Write-Host ""
if ($codigo -eq 0) {
  Write-Host "Listo. Revise los cursos en el panel: quedan en borrador." -ForegroundColor Green
} else {
